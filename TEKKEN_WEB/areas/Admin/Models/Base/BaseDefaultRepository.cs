...
{
    public class BaseDefaultRepository : BaseRepository, IBaseDefaultRepository
    {
        private ILogger<BaseDefaultRepository> _logger;

        public BaseDefaultRepository(IConfiguration config, ILogger<BaseDefaultRepository> logger, IHttpContextAccessor httpContextAccessor) : base(config, logger, httpContextAccessor)
        {
        ...
        new DynamicParameters(new { tableName = mainTable.ToString(), id = id, language_code = language_code, BaseType=BaseType.Default.ToString() }),
        commandType: CommandType.StoredProcedure).SingleOrDefault();
        ...
        protected override int SaveOrUpdate(BaseModel baseModel, FormType formType)
        {
            ...
            parameters.Add("@character_code", value: baseModel.Character_code, dbType: DbType.Int16);
