...
{
    public class BaseStateGroupRepository : BaseRepository, IBaseStateGroupRepository
    {
        private ILogger<BaseStateGroupRepository> _logger;

        public BaseStateGroupRepository(IConfiguration config, ILogger<BaseStateGroupRepository> logger, IHttpContextAccessor httpContextAccessor) : base(config, logger, httpContextAccessor)
        {
        ...
        public BaseModel GetRecentBaseModel(int stateGroup_code) => con.Query<BaseModel>("[Base_GetNewNumber]",
        new DynamicParameters(new { tableName = mainTable.ToString(), stateGroup_code = stateGroup_code }),
        commandType: CommandType.StoredProcedure).SingleOrDefault();

        public new BaseModel GetDetailBaseModelById(int id) => con.Query<BaseModel>("[Base_GetDetailById]",
        new DynamicParameters(new { tableName = mainTable.ToString(), id = id, language_code = language_code, BaseType = BaseType.StateGroup_Code.ToString() }),
        commandType: CommandType.StoredProcedure).SingleOrDefault();
        ...
        protected override int SaveOrUpdate(BaseModel baseModel, FormType formType)
        {
            ...
            parameters.Add("@stateGroup_code", value: baseModel.StateGroup_code, dbType: DbType.Int32);
